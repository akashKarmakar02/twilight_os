...
impl Device {
    ...
    fn reset(&mut self) {
        ...
        // wait(500); // TODO: How long should we wait?
        //
    ...
    fn init_rx(&mut self) {
        ...
        let phys_addr = sys::memory::phys_addr(ptr);
    ...
    fn read_mac(&self) -> EthernetAddress {
        ...
        if self.has_eeprom {
            ...
            unsafe {
                ...
                let addr = sys::memory::phys_to_virt(phys).as_u64();
    ...
    fn read(&self, addr: u16) -> u32 {
        unsafe {
            if self.bar_type == 0 {
                ...
                let addr = sys::memory::phys_to_virt(phys).as_u64() as *mut u32;
    ...
    fn debug(&self) {
        // Registers
        println!("NET E1000: ICR:    {:#034b}", self.read(REG_ICR));
        println!("NET E1000: CTRL:   {:#034b}", self.read(REG_CTRL));
        println!("NET E1000: STATUS: {:#034b}", self.read(REG_STATUS));
        println!("NET E1000: RDH -> {}", self.read(REG_RDH));
        println!("NET E1000: RDT -> {}", self.read(REG_RDT));
        println!("NET E1000: TDH -> {}", self.read(REG_TDH));
        println!("NET E1000: TDT -> {}", self.read(REG_TDT));

        // Receive descriptors
        let rx_descs = self.rx_descs.lock();
        for i in 0..RX_BUFFERS_COUNT {
            let ptr = ptr::addr_of!(rx_descs[i]) as *const u8;
            let phy = sys::memory::phys_addr(ptr);
            println!(
                "NET E1000: [{}] {:?} ({:#X} -> {:#X})",
...
impl EthernetDeviceIO for Device {
    ...
    fn receive_packet(&mut self) -> Option<Vec<u8>> {
        ...
        if rx_descs[rx_id].status & RSTA_DD > 0 {
            ...
            self.rx_id
                .store((rx_id + 1) % RX_BUFFERS_COUNT, Ordering::SeqCst);
