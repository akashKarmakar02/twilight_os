...
const SECTORS_PER_CLUSTER: u32 = 4; // 2 KiB clusters
...
pub fn format_partition(
    ...
    let layout = Fat16Layout::new(start_lba, sector_count)?;
    builder.assign_short_names();
    builder.compute_directory_clusters(layout.cluster_size_bytes)?;

    let mut formatter = Fat16Formatter::new(device, layout, builder)?;
...
impl Fat16Layout {
    fn new(start_lba: u32, total_sectors: u32) -> Result<Self, FatError> {
        ...
        Ok(Self {
            ...
            root_dir_lba: start_lba + u32::from(RESERVED_SECTORS) + NUM_FATS * fat_size,
            root_dir_sectors,
            total_clusters,
...
impl<'a, 'b> Fat16Formatter<'a, 'b> {
    ...
    fn process_directory(
        ...
        if dir_id == 0 {
            self.builder.directories[dir_id].cluster = Some(0);
        } else if self.builder.directories[dir_id].cluster.is_none() {
            let start_cluster =
    ...
    fn write_root_dir(&mut self, buf: &[u8]) -> Result<(), FatError> {
        let bytes = self.layout.root_dir_sectors as usize * partition::SECTOR_SIZE as usize;
        let mut padded = vec![0u8; bytes];
        let len = cmp::min(bytes, buf.len());
        padded[..len].copy_from_slice(&buf[..len]);

        let mut lba = self.layout.root_dir_lba;
        for chunk in padded.chunks(partition::SECTOR_SIZE as usize) {
            self.device
                .write(lba, chunk)
    ...
    fn write_boot_region(&mut self) -> Result<(), FatError> {
        ...
        sector[19..21].copy_from_slice(&(self.layout.total_sectors as u16).to_le_bytes());
        sector[21] = 0xF8;
        sector[22..24].copy_from_slice(&self.layout.fat_size_sectors.to_le_bytes());
        sector[24..26].copy_from_slice(&0x20u16.to_le_bytes());
        sector[26..28].copy_from_slice(&0x40u16.to_le_bytes());
        sector[28..32].fill(0);
        sector[32..36].copy_from_slice(&(self.layout.total_sectors as u32).to_le_bytes());
        sector[36..40].fill(0);
        sector[40] = 0x80;
        sector[41] = 0x00;
        sector[42] = 0x29;
        sector[43..47].copy_from_slice(&0xdeadbeefu32.to_le_bytes());
        sector[47..58].copy_from_slice(b"TWILIGHTOS ");
        sector[58..66].copy_from_slice(b"FAT16   ");
        sector[510] = 0x55;
        sector[511] = 0xAA;
    ...
    fn write_fats(&mut self) -> Result<(), FatError> {
        ...
        for (idx, entry) in self.fat.iter().enumerate() {
            ...
            buffer[offset..offset + 2].copy_from_slice(&entry.to_le_bytes());
        }
...
fn compute_fat16_size(total_sectors: u32, root_dir_sectors: u32) -> Result<(u32, u32), FatError> {
    let reserved = u32::from(RESERVED_SECTORS);
    let mut fat_size = 1u32;
    loop {
        let data_sectors = total_sectors
            .checked_sub(reserved + root_dir_sectors + NUM_FATS * fat_size)
