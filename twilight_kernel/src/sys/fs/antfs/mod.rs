...
pub use crate::api::fs::{FileIO, IO, dirname, filename, realpath};
...
pub fn resolve_relative(dirfd: &str, path: &str) -> Result<String, ()> {
    // Get the directory path associated with dirfd
    let _ = match info(dirfd) {
        Some(p) => p,
        None => return Err(()),
    };

    let full_path = if path.starts_with('/') {
        // Absolute path, use canonicalize directly
        path.to_string()
    } else {
        // Relative path, prepend dir_path
        format!("{}/{}", dirfd, path)
    };

    // Canonicalize the final path to handle '~' and return
